...
def class_defined(context, clazz):
    ...
    assert isinstance(getattr(context.under_test, clazz), type), f"{clazz} is not a class"
...
def function_takes_args(context, func, args):
    ...
    assert num_args == args, f"{func} does not have {args} positional parameters, it has {num_args} ({positional_parameters})"
...
def subclazz_inherits_from_clazz(context, subclazz, clazz):
    ...
    assert issubclass(getattr(context.under_test, subclazz), getattr(context.under_test, clazz)), f"{subclazz} does not inherit from {clazz}"

@given('{subclazz:w} class inherits from tk.{clazz:w}')
def subclazz_inherits_from_tk_clazz(context, subclazz, clazz):
    assert hasattr(context.under_test, subclazz), f"{subclazz} not defined"
    assert issubclass(getattr(context.under_test, subclazz), getattr(tk, clazz)), f"{subclazz} does not inherit from {clazz}"
